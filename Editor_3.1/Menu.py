...
class Menu():
	def menus(self):
		self.menu = tk.Menu(self.master,activebackground='grey',)
		self.file = tk.Menu(self.menu,tearoff=0)
		self.file.add_command(label='Salvar',command=self.salvar)
		self.file.add_command(label='Abrir',command=self.abrir)
		self.file.add_command(label='Novo',command=self.novo)
		#	self.file.add_command(label='layout',command=self.layout)
		self.editor = tk.Menu(self.master,tearoff = 0)
		self.editor.add_command(label='foreground',command=self.foreground_text)
		self.editor.add_separator()
		self.editor.add_command(label='background',command=self.background_text)
		self.menu.add_cascade(label='File',menu=self.file)
		self.menu.add_cascade(label="color",menu=self.editor)
	...
	def cores(self):
		self.cor= {
			...
			'deep_purple': ['#EDE7F6', '#D1C4E9', '#B39DDB', '#9575CD', '#7E57C2', '#673AB7', '#5E35B1', '#512DA8',
			...
			'light_blue': ['#E1F5FE', '#B3E5FC', '#81D4FA', '#4FC3F7', '#29B6F6', '#03A9F4', '#039BE5', '#0288D1',
			...
			'light_green': ['#F1F8E9', '#DCEDC8', '#C5E1A5', '#AED581', '#9CCC65', '#8BC34A', '#7CB342', '#689F38',
			...
			'deep_orange': ['#FBE9E7', '#FFCCBC', '#FFAB91', '#FF8A65', '#FF7043', '#FF5722', '#F4511E', '#E64A19',
							'#D84315', '#BF360C', '#FF9E80', '#FF6E40', '#FF3D00', '#DD2C00'],
			'brown': ['#EFEBE9', '#D7CCC8', '#BCAAA4', '#A1887F', '#8D6E63', '#795548', '#6D4C41', '#5D4037',
					  '#4E342E', '#3E2723'],
			'grey': ['#FAFAFA', '#F5F5F5', '#EEEEEE', '#E0E0E0', '#BDBDBD', '#9E9E9E', '#757575', '#616161',
					 '#424242', '#212121'],
			'blue_grey': ['#ECEFF1', '#CFD8DC', '#B0BEC5', '#90A4AE', '#78909C', '#607D8B', '#546E7A', '#455A64',
						  '#37474F', '#263238'],
			'black': ['#000000'],
			'white': ['#FFFFFF'],
		}
		self.valor = [self.valor for self.valor in self.cor.values()]
		self.key = [self.key for self.key in self.cor.keys()]
		
		#for i in range(21):
			#print(self.valor[i])
		for i in range(21):
			pass
			#print(self.key[i], self.valor[i])
		return self.valor
